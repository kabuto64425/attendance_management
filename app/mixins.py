...
class MonthCalendarMixin(BaseCalendarMixin):
    ...
    def get_month_calendar(self):
        ...
        calendar_data = {
            ...
            'month_stampings': self.get_month_days(current_month),
        }
...
class MonthWithScheduleMixin(MonthCalendarMixin):
    ...
    def get_month_schedules(self, start, end, days):
        ...
        queryset1 = self.model.objects.filter(stamping_by = self.request.user, stamping__range=[start, new_end], stamping_type = 0).order_by('stamping')
        queryset2 = self.model.objects.filter(stamping_by = self.request.user, stamping__range=[start, new_end], stamping_type = 1).order_by('stamping').reverse()
        #Stamping.objects.filter(stamping_by = self.request.user, stamping_type = 0,stamping__date=today).order_by('stamping')

        # {1日のdatetime: 1日のスケジュール全て, 2日のdatetime: 2日の全て...}のような辞書を作る
        day_schedules = {day: {} for week in days for day in week}
        for schedule in queryset1:
            schedule_date = getattr(schedule, self.date_field)
            key = localtime(schedule_date).date()
            day_schedules[key].setdefault(0, schedule)

        for schedule in queryset2:
            schedule_date = getattr(schedule, self.date_field)
            key = localtime(schedule_date).date()
            day_schedules[key].setdefault(1, schedule)

        #for week in days:
        #    for day in week:
        #        print(day)
        #        day_schedules.setdefault(day, None)
